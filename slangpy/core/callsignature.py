...
def generate_code(
    ...
    # call_group_id, call_group_thread_id and the flattened versions of these.
    ...
    if call_data_len > 0:
        ...
        # 1-dimensional id.
        cg.add_snippet("flat_call_id", f"static int flat_call_id = 0;\n")
        cg.add_snippet("flat_call_group_id", f"static uint flat_call_group_id = 0;\n")
        cg.add_snippet("flat_call_group_thread_id", f"static uint flat_call_group_thread_id = 0;\n")
        ...
        cg.add_snippet(
            "get_flat_call_id", "export public int get_flat_call_id() { return flat_call_id; };\n"
        )
        cg.add_snippet(
            "get_flat_call_group_id",
            "export public uint get_flat_call_group_id() { return flat_call_group_id; };\n",
        )
        cg.add_snippet(
            "get_flat_call_group_thread_id",
            "export public uint get_flat_call_group_thread_id() { return flat_call_group_thread_id; };\n",
        )
    ...
    # global-scope static variables such that they can be "queried" in user shaders.
    if call_data_len > 0:
        # Calculate flattened id's that will be used to determine the N-dimensional
        # id's.
        cg.kernel.append_statement(f"flat_call_id = flat_call_thread_id.x")
        ...
        cg.kernel.append_statement(f"flat_call_group_id = flat_call_thread_id.x / {call_group_size}")
        ...
        cg.kernel.append_statement(
            f"flat_call_group_thread_id = flat_call_thread_id.x % {call_group_size}"
        )
    ...
    if call_data_len > 0:
        ...
        # We omit flat_call_thread_id here as it is already added above.
        # Args here are used to support calls like spy.call_id(), spy.call_group_id(), etc
