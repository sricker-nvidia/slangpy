...
"""
Comprehensive Integration Tests for call_group_shape Functionality

This module provides extensive integration testing of the call_group_shape feature
with all major SlangPy components, ensuring robust interoperability and correctness.
...
works seamlessly across SlangPy's ecosystem.
...
Special attention to texture format compatibility:
- Multi-channel textures (RGBA32_float) for complex operations
- Single-channel textures (R32_float) with proper vector type handling
- Proper usage flag management (shader_resource vs unordered_access)

### Error Scenarios Covered
- Shape mismatches between call groups and data dimensions
- Texture usage binding validation
- Broadcasting compatibility with strict mode
- Memory layout considerations for different buffer types

## Requirements
- SlangPy with call_group_shape support
- Multi-device backend support (D3D12/Vulkan)
- NumPy for data validation
- pytest for test execution

## Usage
Run all integration tests:
    pytest test_call_group_integrations.py -v

Run specific integration category:
    pytest test_call_group_integrations.py -k "tensor" -v
    pytest test_call_group_integrations.py -k "texture" -v
    pytest test_call_group_integrations.py -k "ndbuffer" -v
    pytest test_call_group_integrations.py -k "transform" -v
"""
...
def test_tensor_autodiff_with_call_groups(device_type: DeviceType):
    ...
    result_tensor.grad_in.storage.copy_from_numpy(
